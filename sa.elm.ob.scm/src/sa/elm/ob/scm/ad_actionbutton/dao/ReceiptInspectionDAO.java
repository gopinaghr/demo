...
public class ReceiptInspectionDAO {
  ...
  public JSONObject getInspectedRecord(String clientId, String inpReceiptId, JSONObject searchAttr) {

    JSONObject resultObject = new JSONObject(), json = null, json1 = null;
    StringBuilder countQuery = new StringBuilder(), selectQuery = new StringBuilder(), fromQuery = new StringBuilder(), whereClause = new StringBuilder(), groupClause = new StringBuilder(), orderClause = new StringBuilder();
    ...
    try {
      ...
      countQuery
          .append("select totalRecord as totalRecord from (select count(distinct ins.escm_initialreceipt_id) as totalRecord ");
      selectQuery
          .append(" select line,id,id1,product,initialQty,status,qty,inspectiondate,qualitycode,inspectedby,notes,uom,m_product_id from ");
      selectQuery
          .append("( select rec.line,rec.escm_initialreceipt_id as id1 ,ins.escm_inspection_id as id ,prd.name as product,prd.m_product_id , (rec.quantity-coalesce(rec.ir_return_qty,0)) as initialQty,case when ins.status='A' then "
              + "'Accept' else 'Reject' end as status ");
      selectQuery
          .append(" ,ins.quantity as qty,(select eut_convert_to_hijri(to_char(ins.inspectiondate,'yyyy-MM-dd'))) as inspectiondate ,ins.qualitycode,ins.inspectedby,ins.notes,uom.name as uom  ");
      fromQuery
          .append("from escm_inspection ins left join  escm_initialreceipt  rec on rec.escm_initialreceipt_id=ins.escm_initialreceipt_id ");
      fromQuery
          .append("left join m_product prd on prd.m_product_id =rec.m_product_id left join c_uom uom on uom.c_uom_id=rec.c_uom_id  where 1=1  and prd.em_escm_noinspection='N' and producttype='I' and isstocked='Y' ");
      ...
      while (rs.next()) {
        ...
        if (tempProductId != "" && tempProductId.equals(rs.getString("m_product_id"))) {
