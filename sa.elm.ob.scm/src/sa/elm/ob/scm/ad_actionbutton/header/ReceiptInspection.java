        ...
        if (receiptId != null) {
          for (int i = 0; i < jsonArray.length(); i++) {
            ...
            if (inspection != null) {
              ...
              if (json.getString("notes") != null && json.getString("notes") != "")
                inspection.setNotes(json.getString("notes"));
              if (json.getString("qty") != null && json.getString("qty") != "")
                inspection.setQuantity(new BigDecimal(json.getString("qty")));
              log4j.debug("inspectiondate" + json.getString("inspectiondate") != "");
              log4j.debug("inspectiondate" + json.getString("inspectiondate") != " ");
              if (json.getString("inspectiondate") != null
                  && json.getString("inspectiondate") != "") {
                inspection.setInspectionDate(employeedao.convertGregorian(json.getString(
                    "inspectiondate").toString()));
              }
              if (json.getString("inspectedby") != null && json.getString("inspectedby") != "")
                inspection.setInspectedBy(json.getString("inspectedby"));
              if (json.getString("qualitycode") != null && json.getString("qualitycode") != "")
                inspection.setQualityCode(json.getString("qualitycode"));
              if (json.getString("isinspected") != null && json.getString("isinspected") != "") {
