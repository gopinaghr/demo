...
public class ESCMSiteReceiveAddLinesInjection extends HqlQueryTransformer {
  ...
  protected StringBuffer getSelectClause(boolean hasSelectedIds,
    ...
    selectClause
        .append("  receive.client.id as ad_client_id,receive.organization.id as ad_org_id,   receive.active  as isactive, receive.documentNo as documentNo,receive.createdBy.id as createdBy , "
    ...
    if (headerId != null) {
      selectClause
          .append(" , coalesce( ( select e.requestedQty from Escm_Material_Reqln e left join e.escmMaterialRequest req "
              + " where req.id='"
              + headerId
  ...
  protected StringBuffer getWhereClause(Map<String, String> requestParameters,
    ...
    String headerId = requestParameters.get("@Escm_Material_Request.id@");
    Boolean ispreference = Preferences.existsPreference("ESCM_LineManager", true, null, null, null,
        vars.getRole(), null);
