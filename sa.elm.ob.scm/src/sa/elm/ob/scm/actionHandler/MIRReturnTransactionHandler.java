...
public class MIRReturnTransactionHandler extends BaseActionHandler {
  ...
  protected JSONObject execute(Map<String, Object> parameters, String content) {
    ...
    try {
      ...
      System.out.println("mRequestLineId:" + mRequestLineId);
      ...
      if (Integer.valueOf(selectedlines.length()) != objRequestLine.getDeliveredQantity()
        ...
        successMessage.put(
            "text",
            OBMessageUtils.messageBD("Escm_Issued_Qty_Less").replace("@",
      ...
      if (selectedlines.length() > 0) {
        for (int a = 0; a < selectedlines.length(); a++) {
          ...
          ps = conn
              .prepareStatement(" select coalesce(max(line),0)+10 as lineno from escm_custody_transaction where escm_material_reqln_id  = ? ");
          ...
          custtransaction.setTransactionreason(Resource.getProperty("scm.IssueReturnMIR",
              vars.getLanguage()));
