    ...
    try {
      ...
      if (selectedlines.length() > 0) {
        for (int a = 0; a < selectedlines.length(); a++) {
          ...
          ps = conn
              .prepareStatement(" select coalesce(max(line),0)+10 as lineno from escm_custody_transaction where m_inoutline_id  = ? ");
          ...
          if (inout.getEscmReceivingtype().equals("INR"))
            ...
            ps1 = conn
                .prepareStatement(" select list.name  from ad_ref_list list where ad_reference_id='A692FD4437F84F1A990B7A60B058E9A8'"
                    + " and list.value = ?");
