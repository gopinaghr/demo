      ...
      if (selectedlines.length() > 0) {
        ...
        if (request.getAlertStatus().equals("DR") && (!roleID.equals(vars.getRole()))) {
          if (request.getEUTNextRole() == null) {
            OBQuery<MaterialIssueRequestHistory> custransa = OBDal.getInstance().createQuery(
                ...
                " as e where e.escmMaterialRequest.id='" + request.getId()
        ...
        for (int a = 0; a < selectedlines.length(); a++) {
          ...
          OBQuery<MaterialIssueRequestLine> chklineexistQry = OBDal.getInstance().createQuery(
              ...
              "as e where e.escmMaterialRequest.id='" + MaterialReqId + "' "
                  + "and e.escmInitialreceipt.id ='" + selectedRow.getString("id") + "'");
          ...
          if (chklineexistQry.list().size() > 0) {
            ...
            if (selectedRow.getString("reqqty") != null
              ...
              EscmInitialReceipt objInitialReceipt = OBDal.getInstance().get(
                  EscmInitialReceipt.class, selectedRow.getString("id"));
              BigDecimal availbleQty = objInitialReceipt.getQuantity().subtract(
                  objInitialReceipt.getSitereqissuedqty());
            ...
            if (selectedRow.getString("reqqty") != null
                ...
                && new BigDecimal(selectedRow.getString("reqqty")).compareTo(BigDecimal.ZERO) > 0) {
              ps = conn
                  .prepareStatement(" select coalesce(max(line),0)+10 as lineno from escm_material_reqln where escm_material_request_id=?");
