  ...
  protected JSONObject execute(Map<String, Object> parameters, String content) {
    ...
    try {
      ...
      for (int a = 0; a < selectedlines.length(); a++) {
        ...
        if (selectedRow.getString("storageBin") == null
            || selectedRow.getString("storageBin") == ""
            || selectedRow.getString("storageBin") == "null") {
          defaultBinId = Utility.GetDefaultBin(warehouse);
          if (defaultBinId.equals("")) {
            OBDal.getInstance().rollbackAndClose();
            JSONObject errorMessage = new JSONObject();
        ...
        ps = conn
            .prepareStatement(" select coalesce(max(line),0)+10 as lineno from m_inventoryline where m_inventory_id=?");
