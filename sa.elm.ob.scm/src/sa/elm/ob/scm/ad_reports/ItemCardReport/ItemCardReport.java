  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (action.equals("")) {
        ...
        st = con
            .prepareStatement(" select name,m_warehouse_id from m_warehouse where ad_client_id= ?  and em_escm_warehouse_type='MAW' and ad_org_id in( select ad_org_id  from ad_role_orgaccess   where ad_role_id  = ? and ad_client_id = ? )    and isactive='Y'  order by name asc  ");
        ...
        st = con
            .prepareStatement(" select (value||'-'||name) as name ,m_product_id from m_product where ad_client_id= ?    and isactive='Y'  order by name asc  ");
        st.setString(1, inpClientId);
        rs = st.executeQuery();
        while (rs.next()) {
          vo = new CustodyCardReportVO();
        ...
        st = con
            .prepareStatement(" select m_locator_id as id ,value from m_locator where m_warehouse_id= ?  and ad_client_id=?   and isactive='Y' order by value asc ");
