  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (action.equals("")) {
        ...
        st = con
            .prepareStatement(" select name, m_warehouse_id from m_warehouse where em_escm_warehouse_type='MAW' "            		
        ...
        st = con
            .prepareStatement(" select (prd.value||'-'||prd.name||'-'||stk.name) as name, m_product_id from m_product prd "
            					+" join escm_deflookups_typeln stk on stk.escm_deflookups_typeln_id=prd.em_escm_stock_type "
            					+" where prd.isactive='Y' and prd.ad_org_id in ( select ad_org_id  from ad_role_orgaccess   where ad_role_id  = ? and ad_client_id = ? ) order by name asc");
        st.setString(1, inpRoleId);
        st.setString(2, inpClientId);        
        rs = st.executeQuery();
        while (rs.next()) {
        ...
        if(inpWarehouseId.equals("0"))
        	...
        	parameters.put("inpWarehouseId", " and warehouse.m_warehouse_id = '"+inpWarehouseId+"' ");
