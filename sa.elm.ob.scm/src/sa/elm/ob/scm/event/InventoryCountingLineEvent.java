...
public class InventoryCountingLineEvent extends EntityPersistenceEventObserver {
  ...
  private static Entity[] entities = { ModelProvider.getInstance().getEntity(
      InventoryCountLine.ENTITY_NAME) };
    ...
    try {
      ...
      if (currentLine.getPhysInventory().getEscmCounttype() != null
        ...
        if (!event.getCurrentState(sotrageBin).equals(event.getPreviousState(sotrageBin))
            ...
            || (event.getPreviousState(attributeset) != null && !event
                .getCurrentState(attributeset).equals(event.getPreviousState(attributeset)))) {

          OBQuery<InventoryCountLine> line = OBDal.getInstance().createQuery(
              InventoryCountLine.class,
              "product.id = '" + currentLine.getProduct().getId() + "' and storageBin.id='"
                  + currentLine.getStorageBin().getId() + "' and uOM.id='"
                  + currentLine.getUOM().getId() + "' and physInventory.id='"
                  + currentLine.getPhysInventory().getId() + "'");
