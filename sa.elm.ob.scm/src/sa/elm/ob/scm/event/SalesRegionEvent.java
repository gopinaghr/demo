...
public class SalesRegionEvent extends EntityPersistenceEventObserver {
  ...
  public void onDelete(@Observes EntityDeleteEvent event) {
    ...
    try {
      ...
      OBQuery<MaterialIssueRequest> matiss = OBDal.getInstance().createQuery(
          MaterialIssueRequest.class, " as e where e.beneficiaryIDName='" + dept.getId() + "' ");
      if (matiss.list().size() > 0) {
        throw new OBException(OBMessageUtils.messageBD("ForeignKeyViolation"));
      }
      OBQuery<ShipmentInOut> receipt = OBDal.getInstance().createQuery(
          ShipmentInOut.class,
          " as e where ( e.escmBname='" + dept.getId() + "' or e.escmTobenefiName='" + dept.getId()
              + "')");
