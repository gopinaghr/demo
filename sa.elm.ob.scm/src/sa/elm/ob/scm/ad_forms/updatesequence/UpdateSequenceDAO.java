...
public class UpdateSequenceDAO {
	...
	public synchronized JSONObject getOrgList(String clientId, String roleId) {
	    ...
	    try {
	      ...
	      fromQuery
	          .append(" from ad_org org left join ad_orgtype orgtyp on orgtyp.ad_orgtype_id=org.ad_orgtype_id "
	    		  +" where org.ad_org_id<>'0' and org.ad_org_id in (select ad_org_id from ad_role_orgaccess rol where rol.ad_role_id='"+roleId+"') "
	    		  +" and istransactionsallowed='Y' and isready='Y' ");
			      ...
			      if (sequenceQuery.list().size() > 0) {
			        for (DocumentSequence objSequence : sequenceQuery.list()) {
			          if (new BigDecimal(objSequence.getPrefix())
			              .compareTo(new BigDecimal(prefix)) == 1) {
