...
public class NonCustodyUploadTemplateDAO {
  ...
  Logger log4j = Logger.getLogger(NonCustodyUploadTemplateDAO.class);
  ...
  public JSONObject processUploadedCsvFile(String filePath) {
    ...
    try {
      ...
      if (isSuccess == 1) {
        ...
        OBCriteria<CsvNoncustodyImport> csvTable = OBDal.getInstance().createCriteria(
            CsvNoncustodyImport.class);
      ...
      while ((inpLine = inpReader.readLine()) != null) {
        ...
        if (fields.length < 4) {
          ...
          if (StringUtils.isEmpty(fields[1])) {
            failed += 1;
            description.append("Quantity is empty at line No.<b>" + lineNo
                + "</b> in the data file.<br>");
            br = "<br>";
          }
