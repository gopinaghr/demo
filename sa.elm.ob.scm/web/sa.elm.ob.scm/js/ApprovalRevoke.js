...
function onClickRefresh() {
	 var action = "";
  	 document.frmMain.action =contextPath+'/sa.elm.ob.scm.ad_forms.ApprovalRevoke.header/ApprovalRevoke?inpAction='+action+'&inpWindowId='+document.getElementById("inpWindowId").value;
...
function revokeReocrds() {
	 ...
	 if(selectIds===''){
		 OBAlert("Select Atleast one record to revoke");
	 }
	   OBAsk("Are You Sure to Revoke ?", function(result) {
		   if(result){
				document.frmMain.action = contextPath+'/sa.elm.ob.scm.ad_forms.ApprovalRevoke.header/ApprovalRevoke?inpAction=Revoke&selectIds='+selectIds+'&inpWindowId='+inpWindowId;
			    document.frmMain.submit();
		   }
	   });
}