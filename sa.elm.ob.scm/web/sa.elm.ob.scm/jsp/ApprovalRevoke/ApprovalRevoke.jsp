...
<%@page import="sa.elm.ob.scm.ad_forms.ApprovalRevoke.vo.ApprovalRevokeVO"%>
  ...
  <HEAD>
   <META http-equiv="Content-Type" content="text/html; charset=utf-8"></META> 
    ...
    function onLoad()
    {
        ...
        { %>displayMessage("S", "<%=Resource.getProperty("scm.success",lang)%>","<%= Resource.getProperty("scm.process.success",lang) %>");<% } 
        else if(request.getAttribute("SaveStatus").equals("Error")) 
        {
        %>displayMessage("E", "<%=Resource.getProperty("scm.error",lang)%>","<%= Resource.getProperty("scm.process.error",lang) %>") <%
        }
        else if(request.getAttribute("SaveStatus").equals("AlreadyProcessed")) 
        {
         %>displayMessage("E", "<%=Resource.getProperty("scm.error",lang)%>","<%= Resource.getProperty("scm.process.document",lang) %> <%= request.getAttribute("DocumentNo") %> <%= Resource.getProperty("scm.process.alreadyprocessed",lang) %>") <%
        }%>
        onResizeDo();
    }
    </script>
                        ...
                        <table class="Main_ContentPane_ToolBar Main_ToolBar_bg" id="tdToolBar">
                            ...
                            <td style="width: 100%;"><div style="display: inline-block; position: relative; top: 8px; right: 30px; float: right;">
                                ...
                                <tbody><tr>
                                    ...
                                    <td id="Submit_BTNname" class="Button_text"><%= Resource.getProperty("scm.revoke", lang) %></td>
                    ...
                    <TABLE width="100%" border="0" cellspacing="0" cellpadding="0" class="Main_ContentPane_TabPane" id="tdtopTabs">
                          ...
                          <tr><td class="tabBackGround">
                            ...
                            <div><span class="dojoTabcurrentfirst"><div>
                                <span><a class="dojoTabLink" href="javascript:void(0);" onclick="return false;" onMouseOver="return true;" onMouseOut="return true;"><%= Resource.getProperty("scm.mass.revoke.records",lang)%></a></span>
                            </div></span></div>
                            </div>
                        </td></TR>
                    </TABLE>
                                ...
                                <TABLE class="dojoTabPaneWrapper" border="0" cellpadding="0" cellspacing="0" width="100%" summary="">
                                    <TR><TD>
                                                                    ...
                                                                    <TABLE class="MessageBox_Top"><TBODY>
                                                                        <TR><TD>
                                                                            ...
                                                                            <div id="hideMessage" style="float: right; margin-top:-13px; margin-right:10px;">
                                                                                <a style="color: yellow;font-size: 11.5px" href="javascript:void(0);" onClick="document.getElementById('messageBoxID').style.display = 'none';"><%=Resource.getProperty("scm.hide",lang)%></a>
                                                                            </div>
                                                                     </TD></TR>
                                                                     </TBODY></TABLE>
                                                                </TD>
                                        ...
                                        <div id="LoadingContent" style="display:none;">
                                            ...
                                            <div style="color:#000000;font-family: sans-serif;font-size:14px;margin-top:10px;text-align: center;"><%= Resource.getProperty("scm.loading", lang)%>... </div>
                                        </div>
                                                ...
                                                <div style="width:100%;" id="FormDetails">
                                                    <div align="left">
                                                        ...
                                                        <!-- /*<<<<<Your Code Here>>>>>*/ -->
                                                          ...
                                                          <TR>
                                                            <TD class="TitleCell" align="left" style="min-width:110px;"><span class="LabelText" ><%= Resource.getProperty("scm.Windows",lang)%></span></TD>
                                                            <TD class="TextBox_ContentCell">
                                                            <select id="inpWindowId" name="inpWindow" class="ComboKey Combo_TwoCells_width" onchange="onchangeWindow();">
                                                            <option value="MIR"><%= Resource.getProperty("scm.MIR", lang) %></option>
                                                            <option value="SIR"><%= Resource.getProperty("scm.SIR", lang) %></option>  
                                                            <option value="CT">Custody Transfer</option>  
                                                            <option value="RT">Return Transaction</option>  
                                                            </select></TD>
                                                          </TR>
                                                        </TABLE >
                                                    </div>
...
<script type="text/javascript">
var Organization='<%= Resource.getProperty("scm.organization", lang) %>',
inpWindowId='<%= request.getAttribute("inpWindowId") %>'
SpecNo='<%= Resource.getProperty("scm.spec.no", lang) %>',
Requester='<%= Resource.getProperty("scm.requester", lang) %>',
NextApprover='<%= Resource.getProperty("scm.NextApprover", lang) %>',
LastActionPerformer='<%= Resource.getProperty("scm.LastActionPerformer", lang) %>',
Status='<%= Resource.getProperty("scm.Status", lang) %>';
...
<script type="text/javascript" src="../web/sa.elm.ob.scm/js/ApprovalRevoke.js"></script>
